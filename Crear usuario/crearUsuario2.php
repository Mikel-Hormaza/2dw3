...
try {
    ...
    if ($contraseña == $contraseña2) {
        ...
        /*$sqlnom = "SELECT COUNT(*) FROM usuario WHERE nomUsuario='$nombre'";
        $sqlcorreo = "SELECT COUNT(*) FROM usuario WHERE correoUsuario='$correo'";
        $sqlpass = "SELECT COUNT(*) FROM usuario WHERE passUsuario='$contraseña'";

        if ($resultadonom = $konexioa->query($sqlnom)) {
            if ($resultadonom->fetchColumn() > 0) {
        ...
        if ($resultadocorreo = $konexioa->query($sqlcorreo)) {
            if ($resultadocorreo->fetchColumn() > 0) {
        ...
        if ($resultadopass = $konexioa->query($sqlpass)) {
            if ($resultadopass->fetchColumn() > 0) {
                echo "Ya existe un usuario con esa contraseña";
            } else {
                $sql="INSERT INTO usuario (nomUsuario, passUsuario, correoUsuario, permisoUsuario)
                VALUES ('$nombre', '$contraseña', '$correo', 'usuario');";
                $konexioa->exec($sql);
