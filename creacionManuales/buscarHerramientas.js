...
function inicio() {
    ...
    document.getElementById("idCodHerramienta").addEventListener("keyup", comprobarSiHayElementos);
}
...
function filtra() {
    if (document.getElementById("idCodHerramienta").value.length > 0) {
        //recorro el array y si algún string CONTIENE el texto, lo muestro
        for (i = 0; i < arrayOpciones.length; i++) {
            if (arrayOpciones[i].includes(document.getElementById("idCodHerramienta").value)) {
                //Crear elementos
...
function leerPHP() {
    let arrayNombresHerramientas = document.getElementsByName("spanNombresHerramientas").innerHTML;
