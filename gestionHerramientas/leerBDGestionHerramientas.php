...
function llamarBD($where, $primeraVariableLimit, $AscODesc)
{
    ...
    try {
        ...
        $sqlManuales = "SELECT manual.codManual, nombreManual, herramienta.fotoHerramienta, herramienta.codHerramienta, nombreHerramienta
            FROM manual,herramienta " . $where . "
            ORDER BY fechaCreacion  $AscODesc, codHerramienta
            LIMIT $primeraVariableLimit, $maxLimit";
        $resultadoManuales = $conexion->query($sqlManuales);

        $sqlNumManuales = "SELECT herramienta.codHerramienta, manual.codManual
            FROM manual, herramienta " . $where . "
            ORDER by fechaCreacion, codHerramienta ";
...
{
    $_SESSION['codigoDelPrimerManualDeLaTablaMostrado'] = $datosManuales[0]["codHerramienta"];
    $_SESSION['codigoDelUltimoManualDeLaTablaMostrado'] = end($datosManuales)["codHerramienta"];
    $_SESSION['codigoDelUltimoManualDeLaTabla'] = end($datoNumTotalManuales)["codHerramienta"];
    $_SESSION['codigoDelPrimerManualDeLaTabla'] = $datoNumTotalManuales[0]["codHerramienta"];
...
require_once "gestionHerramientas.php";
